...
function draw() {
  ...
  //Assigning functions when stage = 1.
  if(stage === 1) {
    //Displaying text.
    fill("white");
    textSize(20);
    textStyle(BOLD);
    textFont("cursive");
    text("This is a simulator to test the safety and reliability of 4 different cars in the event of a crash",20,50);
    text("with a wall and calculate the damage received by each.",20,80);
    text("Based upon the speed and weight of the car, the deformation (or the damage received) is calculated.",20,130);
    text("~ Any deformation greater than 180 is considered lethal for the passengers.",20,160);
    text("~ Any deformation between 100 and 180 is considered average.",20,190);
    text("~ Deformation below 100 is considered good.",20,220);
    text("Formula for calculating the deformation :",20,270);
    text("Press 'Enter' to start the test.",370,350);
    textSize(23);
    textFont("times new roman");
    text("0.5 * speed * speed * weight / 22500",20,300);

    //Changing stage's value to 2 and playing the start sound when 'enter' key is pressed.
    if(keyDown("enter")) {
      stage = 2;
      start.play();
    }
